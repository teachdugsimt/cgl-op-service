...
interface LambdaServiceServiceProps extends cdk.NestedStackProps {
  apigw: apigateway.RestApi
  secretKey: string
  layer?: lambda.LayerVersion
}

export class LambdaServiceServiceStack extends cdk.NestedStack {
  serviceLambdaFunc: lambda.Function
  serviceIntegration: apigateway.LambdaIntegration

  constructor(scope: cdk.Construct, id: string, props: LambdaServiceServiceProps) {
    ...
    this.serviceLambdaFunc = new lambda.Function(this, 'CglServiceServiceFN', {
      runtime: lambda.Runtime.NODEJS_12_X,
      handler: 'lambda.handler',
      code: lambda.Code.fromAsset('../cgl-op-service-service', {
        exclude: ['src/*', 'test/*']
      }),
      ...
      environment: {
        ...
        "TYPEORM_DATABASE": "service_service",
        "TYPEORM_PORT": port,
    ...
    this.serviceIntegration = new apigateway.LambdaIntegration(this.serviceLambdaFunc)
    const apiGatewayRestApi = props.apigw
    const u0 = apiGatewayRestApi.root.resourceForPath('api/v1/service')
    u0.addProxy({ anyMethod: false }).addMethod('ANY', this.serviceIntegration)

    u0.addMethod('GET', this.serviceIntegration)
    u0.addMethod('POST', this.serviceIntegration)
